...
#Using weco package on dataset1 with WE rules
#Import Data
...
#Automate rule implementation
WE_AD <- function(data, numTS, numnorm){
  a <- list()
  for (i in 1:numTS) {
    b <- weco.combine(data[,i],  sdx = sd(data[1:numnorm, i]),
                      mux = mean(data[1:numnorm, i]),
...
#Using weco package on dataset1 with Nelson rules
#Automate rule implementation but with more rules
Nelson_AD <- function(data, numTS, numnorm){
  a <- list()
  for (i in 1:numTS) {
    b <- weco.combine(data[,i],  sdx = sd(data[1:numnorm, i]),
                      mux = mean(data[1:numnorm, i]),
                      lst.rules = list(list(1), list(2), list(3), list(4), 
...
Nelson_TS1_Class <- as.data.frame(Nelson_AD(TS1, length(TS1), 20))
colnames(Nelson_TS1_Class) <- 1:length(Nelson_TS1_Class)

write.csv(Nelson_TS1_Class,"TimeSeries1_Nelson_Classification.csv")
