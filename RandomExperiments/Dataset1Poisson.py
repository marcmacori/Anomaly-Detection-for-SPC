...
import numpy as np
import pandas as pd 
from DataCreationPoisson import *
...
Df1.to_csv('Data\TimeSeries1Poisson.csv')
Df1_Classification.to_csv('Data\TimeSeries1_Classification_Poisson.csv')


#Creating a poisson dataset for semi-supervised training
def poissonDataSetCreation(mu, sigma, numobs):
    Xs, Ys = [], [],
    for i in range(1000):
        X = poisson(mu, sigma, numobs)
        Y = np.zeros(numobs)
...
TS1, TS_Classification1 = poissonDataSetCreation(10, 1, 60)

# Creating a dataframe and saving .csv
Df1= pd.DataFrame(TS1)
Df1.to_csv('Data\TimeSeriespoisson.csv')