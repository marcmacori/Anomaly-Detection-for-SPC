...
#Equation: y(t)=mu+r(t)*sigma+a*sin(2*pi*t/T) # nolint
...
cyclic <- function(mu, sigma, numobs) {
  return(mu + sigma * rnorm(numobs, 0, 1)
  + runif(numobs, 1.5 * sigma, 2.5 * sigma)
  * sin(2 * seq(1, numobs, by = 1) * pi / 16))
}

#Equation: y(t)=mu+r(t)*sigma+d*(-1)^t # nolint
#d: degree of state departure
systematic <- function(mu, sigma, numobs) {
  return(mu + sigma * rnorm(numobs, 0, 1)
  + runif(numobs, 1 * sigma, 3 * sigma)
  * (-1) ^ seq(1, numobs, by = 1))
}
...
stratification <- function(mu, sigma, numobs) {
  return(mu + sigma * rnorm(numobs, 0, 1)
  * runif(numobs, 0.2 * sigma, 0.4 * sigma))
}

#Equation: y(t)=mu+r(t)*sigma+t*g # nolint
#g: gradient of upward trend
ut <- function(mu, sigma, numobs) {
  return(mu + sigma * rnorm(numobs, 0, 1)
   + seq(1, numobs, by = 1)
   * runif(numobs, 0.05 * sigma, 0.25 * sigma))
  }
#Equation: y(t)=mu+r(t)*sigma-t*g # nolint
...
#Equation: y(t)=mu+r(t)*sigma+k*s # nolint
#k=1 if t>=P, else=0
#P: moment of shift
#s: amplitude of shift
us <- function(mu, sigma, numobs) {
  return(mu + sigma * rnorm(numobs, 0, 1)
  + (runif(numobs, 10, 20) <= seq(1, numobs, by = 1))
  * runif(1, 1 * sigma, 3 * sigma))
}
...
data_creation <- function(mu, sigma, anstart, anend, typeanomaly) {
  if (typeanomaly == "cyclic") {
    return(c(normal(mu, sigma, anstart), cyclic(mu, sigma, anend - anstart)))
  } else if (typeanomaly == "systematic") {
      return(c(normal(mu, sigma, anstart),
      systematic(mu, sigma, anend - anstart)))
  } else if (typeanomaly == "stratification") {
      return(c(normal(mu, sigma, anstart),
       stratification(mu, sigma, anend - anstart)))
...
training_set_creation <- function(mu, sigma, numobs, size) {
  i <- 1
