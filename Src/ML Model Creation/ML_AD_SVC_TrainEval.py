...
sc = {"AUC": "roc_auc", "Accuracy": make_scorer(accuracy_score), "Precision": make_scorer(precision_score),\
     "Recall": make_scorer(recall_score), "F1": make_scorer(f1_score)}


svm_1_eval = GridSearchCV(svm_1, parameters, cv = cv, verbose = 4, scoring = sc, refit="AUC",)
...
svm_2_eval = GridSearchCV(svm_2, parameters, cv = cv, verbose = 4, scoring = sc, refit="AUC",)
...
svm_3_eval = GridSearchCV(svm_3, parameters, cv = cv, verbose = 4, scoring = sc, refit="AUC",)
