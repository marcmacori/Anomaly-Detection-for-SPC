...
def poisson(mu, sigma, numobs):
    return(mu + sigma * np.random.poisson(1, size = (numobs)))
...
# Joining anomly pattern with poisson 1, pattern in specific zones
def TSwithAnomaly(mu, sigma, numobs, anstart, anend, type):
    if (type == "cyclic"):
        a = np.concatenate((poisson(mu, sigma, anstart)\
            , cyclic(mu, sigma, anend - anstart), poisson(mu, sigma, numobs - anend)))
    elif(type == "systematic"):
        a = np.concatenate((poisson(mu, sigma, anstart)\
            , systematic(mu, sigma, anend - anstart), poisson(mu, sigma, numobs - anend)))
    elif(type == "stratified"):
        a = np.concatenate((poisson(mu, sigma, anstart)\
            , stratified(mu, sigma, anend - anstart), poisson(mu, sigma, numobs - anend)))
    elif(type == "ut"):
        a = np.concatenate((poisson(mu, sigma, anstart)\
            , ut(mu, sigma, anend - anstart), poisson(mu, sigma, numobs - anend)))
    elif(type == "dt"):
        a = np.concatenate((poisson(mu, sigma, anstart)\
            , dt(mu, sigma, anend - anstart), poisson(mu, sigma, numobs - anend)))
    return(a)